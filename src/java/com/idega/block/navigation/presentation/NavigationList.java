/*
 * $Id: NavigationList.java,v 1.24.2.3 2007/05/20 21:46:36 gimmi Exp $ Created on 16.2.2005
 * 
...
/**
 * <p>
 * This is the new standard "NavigationList" component, for displaying a list of pages (part of the webtree) within an idegaWeb site to link to. It is
 * based purely on CSS style classes for defining its layout. There is a subclass of this called "NavigationTree" that is based on a older "table"
 * based layout which is now discouraged to use because of Web standards compliance.
 * </p>
...
public class NavigationList extends Block {
	...
	/**
	 * Initialized the main <code>ul</code> tag and adds all page links to the tree.
	 * 
	 * @param iwc
	 * @return
	 */
	...
	private boolean isRootPage(ICTreeNode page) {
		return page.getNodeID() == this._rootPageID;
	}
	...
	/**
	 * Adds the <code>PresentationObject</code> corresponding to the specified <code>PageTreeNode</code> and depth.
	 * 
	 * @param iwc
	 * @param page
	 * @param row
	 * @param depth
	 * @param linkIsDisabled
	 *          TODO
	 * @param table
	 */
	...
	protected String getLocalizedName(ICTreeNode node, IWContext iwc) {
		return (isRootPage(node) && getRootPageName(iwc.getCurrentLocale()) != null) ? getRootPageName(iwc.getCurrentLocale()) : ((PageTreeNode) node).getLocalizedNodeName(iwc);
	}
	...
	/**
	 * Gets whether to order pages for specified depth level alphabetically. Will return the default value if nothing is set for the specified depth.
	 * 
	 * @param depth
	...
	/**
	 * Retrieves the current page as well as the selected page and puts all parents into a <code>Collection</code> to draw the tree with the correct
	...
	/**
	 * Sets the page to use as the root for the tree. If nothing is selected, the root page of the <code>IBDomain</code> is used.
	 * 
	 * @param rootPageID
	 */
	...
	/**
	 * Sets the maximum depth for styles in the <code>NavigationTree</code>. Used to restrict how far down the tree new styles are specified. Set to
	 * -1 by default, meaning no restrictions.
	 * 
	...
	/**
	 * Sets the name of the style to use for links in the stylesheet. Is set by default to a global name, can be altered to allow for individual
	 * settings.
	 * 
	 * @param linkStyleName
	 */
	...
	/**
	 * Sets the name of the style to use for texts in the stylesheet. Is set by default to a global name, can be altered to allow for individual
	 * settings.
	 * 
	 * @param textStyleName
	 */
	...
	/**
	 * <p>
	 * Sets the style class that is rendered out when the property "showForbiddenPagesAsDisabled" is set to true.<br/> The default value is "disabled".
	 * </p>
	 * 
	 * @param styleClass
	 */
	...
	public Object saveState(FacesContext ctx) {
		...
		values[34] = new Boolean(openAllNodes);
		return values;
	}
	...
	public void restoreState(FacesContext ctx, Object state) {
		...
		this.openAllNodes = ((Boolean) values[34]).booleanValue();
	}
	...
	/**
	 * @param displaySelectedPageAsLink
	 *          The displaySelectedPageAsLink to set.
	 */
	...
	/**
	 * @param afterSelectedStyleClass
	 *          The afterSelectedStyleClass to set.
	 */
	...
	/**
	 * @param beforeSelectedStyleClass
	 *          The beforeSelectedStyleClass to set.
	 */
	...
	/**
	 * @param firstChildStyleClass
	 *          The firstChildStyleClass to set.
	 */
	...
	/**
	 * @param firstSelectedStyleClass
	 *          The firstSelectedStyleClass to set.
	 */
	...
	/**
	 * @param lastChildStyleClass
	 *          The lastChildStyleClass to set.
	 */
	...
	/**
	 * @param lastSelectedStyleClass
	 *          The lastSelectedStyleClass to set.
	 */
	...
	public void setRootPageName(Locale locale, String name) {
		if (iRootPageNames == null) {
			iRootPageNames = new HashMap();
		}

		iRootPageNames.put(locale, name);
	}

	private String getRootPageName(Locale locale) {
		if (iRootPageNames != null) {
			return (String) iRootPageNames.get(locale);
		}
	...
	/**
	 * <p>
	 * Sets if an extra (empty) &lt;li&gt; item should be rendered at the end of the list.<br/> This list-item gets by default the style class
	 * extraAfterLastChild.<br/>
	 * </p>
	 * 
	...
	/**
	 * @param extraAfterLastChildStyleClass
	 *          The extraAfterLastChildStyleClass to set.
	 */
